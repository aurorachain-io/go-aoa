...
// information related to the dacchain protocol such als blocks, transactions and logs.
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(emchain.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(emchain.FilterQuery(crit), logs)
	if err != nil {
		return rpc.ID(""), err
	}
