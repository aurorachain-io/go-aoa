...
// See https://github.com/Dacchain/EIPs/issues/176 for the test format specification.
...
func MakePreState(db emdb.Database, accounts core.GenesisAlloc) *state.StateDB {
...
func (tx *stTransaction) toMessage(ps stPostState) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/Dacchain/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
	...
	msg := types.NewMessage(from, to, tx.Nonce, value, gasLimit, tx.GasPrice, data, true, 0, nil, "", nil, "", "")
	return msg, nil
}
