...
import (
	...
	"github.com/Aurorachain-io/go-aoa/common/ntp"
...
// PublicDacchainAPI provides an API to access eminer-pro related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicDacchainAPI struct {
	b Backend
}

// NewPublicDacchainAPI creates a new eminer-pro protocol API.
func NewPublicDacchainAPI(b Backend) *PublicDacchainAPI {
	return &PublicDacchainAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicDacchainAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current eminer-pro protocol version this node supports
func (s *PublicDacchainAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicDacchainAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Dacchain Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Dacchain Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an eminer-pro ECDSA signature for:
// keccack256("\x19Dacchain Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/Dacchain/go-Dacchain/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Dacchain Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/Dacchain/go-Dacchain/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid eminer-pro signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the eminer-pro blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new eminer-pro blockchain API.
...
func (s *PublicBlockChainAPI) GetVotesNumber(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (*big.Int, error) {
	...
	res = res.Div(res, big.NewInt(params.Em))
	return res, state.Error()
}
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	//log.Debug("SendTdxArgs start", "to", args.To, "assetInfo", args.AssetInfo, "args", args)
	...
	if args.To != "" {
		match, err := regexp.MatchString("(?i:^AOA|0x)[0-9a-fA-F]{40}([0-9A-Za-z]{0}|[0-9A-Za-z]{32})$", args.To)
		if err != nil {
			return err
		}
		if !match {
		...
		if len(args.To) > strictAddresLength {
			...
			match, err := regexp.MatchString("(?i:^AOA|0x)[0-9a-fA-F]{40}([0-9A-Za-z]{0}|[0-9A-Za-z]{32})$", subAddress)
			if err != nil {
				return err
			}
			if !match {
	...
	//log.Info("SendTdxArgs start", "to", args.SubAddress)
	if args.Gas == nil {
		args.Gas = new(hexutil.Uint64)
		*(*uint64)(args.Gas) = defaultGas(args.Action)
	}
	//log.Debug("SendTdxArgs Process SubAddress", "to", args.To, "subAddress", args.SubAddress, "gas", *args.Gas)

	//Basic check including gas, gasprice, nonce
...
func countVoteCost(prevVoteList []common.Address, curVoteList []types.Vote, delegateList map[common.Address]types.Candidate) error {
		...
		case 0:
			if _, contain := sliceContains(*vote.Candidate, voteChange); !contain {
				...
				if _, ok := delegateList[*address]; !ok {
					log.Error("vote fail. Vote member not in delegate poll.", "address", address, "delegateList", delegateList)
					return core.ErrVoteList
				}
				voteChange = append(voteChange, *vote.Candidate)
			} else {
				return errors.New("You have already vote candidate " + address.Hex())
			}
		case 1:
			if i, contain := sliceContains(*vote.Candidate, voteChange); contain {
				voteChange = append(voteChange[:i], voteChange[i+1:]...)
			} else {
				return errors.New("You haven't vote candidate " + address.Hex() + " yet")
			}
		default:
...
func (args *SendTxArgs) toTransaction() (*types.Transaction, error) {
	...
	switch args.Action {
	case types.ActionRegister:
		return types.NewRegisterTransaction(uint64(*args.Nonce), uint64(*args.Gas), (*big.Int)(args.GasPrice), args.Action, []byte(args.Nickname)), nil
	...
	case types.ActionAddVote, types.ActionSubVote:
		...
		return types.NewVoteTransaction(uint64(*args.Nonce), uint64(*args.Gas), (*big.Int)(args.GasPrice), args.Action, voteByte), nil
	...
	case types.ActionPublishAsset:
		...
		ai := assetInfoBytes
		return types.NewPublishAssetTransaction(uint64(*args.Nonce), (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), args.Action, ai), nil
	...
	default:
		...
		return types.NewTransaction(uint64(*args.Nonce), common.HexToAddress(args.To), (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), input, args.Action, args.Asset, args.SubAddress), nil
	}
}
...
// PublicDebugAPI is the collection of eminer-pro APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of eminer-pro APIs exposed over the private
// debugging endpoint.
...
// Version returns the current eminer-pro protocol version.
